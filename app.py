...
# --- KUSURSUZ MOBİL UYUMLU, MEB KONSEPTLİ CSS (HATA DÜZELTMELERİ İLE) ---
st.markdown("""
    <style>
    :root { 
        --meb-red: #E30A17; 
    ...
    /* DÜZELTME 2: Mobil taşmaları önlemek için word-break eklendi */
    .metric-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(80px, 1fr)); gap: 10px; margin: 15px 0; }
    .metric-box { background: var(--card-bg); padding: 12px 5px; border-radius: 8px; text-align: center; border: 1px solid #e2e8f0; word-break: break-word; }
    ...
    /* DÜZELTME 3: İndirme butonları için ek stabilite */
    .stDownloadButton > button { width: 100%; margin-top: 0px; }
    </style>
            ...
            if df_idare.empty:
                ...
                with idare_tab1:
                    ...
                    if idare_okul == "Tüm İlçe (Genel)":
                        ...
                        lider_okul = df_grup.iloc[-1]['OKUL ADI'] if not df_grup.empty else "-"
                        lider_puan = df_grup.iloc[-1]['Ort_Puan'] if not df_grup.empty else 0
                        zayif_okul = df_grup.iloc[0]['OKUL ADI'] if not df_grup.empty else "-"
                        ...
                        if idare_sinif == "Tüm Sınıflar Kademesi":
                            ...
                            en_iyi_kademe = df_sinif_grup.sort_values(by='Ort_Puan').iloc[-1]['Sınıf_Gorsel'] if not df_sinif_grup.empty else "-"
                ...
                # ==============================================================================
                # BÖLÜM 4: TOPLU LİSTE VE TOPLU KARNE İNDİRME MERKEZİ (HATA DÜZELTİLDİ)
                # ==============================================================================
                with idare_tab2:
                    ...
                    # --- 2. TOPLU KARNE OLUŞTURMA (DÖNGÜ HATASI DÜZELTİLDİ) ---
                    html_toplu_karne = f"""
                    <html><head><meta charset="utf-8"><style>
                    ...
                    # DÜZELTME 1: İşte esas hata buradaydı. i değişkenini int'e zorluyoruz.
                    for i, row in df_export.reset_index().iterrows():
                        if int(i) % 2 == 0:
                            html_toplu_karne += "<div class='page'>"
                        ...
                        '''
                        if (int(i) + 1) % 2 == 0 or int(i) == len(df_export) - 1:
                            html_toplu_karne += "</div>"
